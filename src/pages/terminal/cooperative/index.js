...
import { Delete_Cooperative, Get_Cooperative, Post_Cooperative, Put_Cooperative } from "../../api/cooperative";
...
export default function Home() {
    ...
    const [selected_puv_type_id, set_selected_puv_type_id] = useState(null);
    const [puv_type_data, set_puv_type_data] = useState([]);
    ...
    const handleGetData = async () => {
        ...
        try {
            ...
            let ApiResponse = await Get_Cooperative()
    ...
    const Handle_Get_Puv_Type_Data = async () => {
        Setisfetching(true)
        ...
        try {
            ...
            set_puv_type_data(ApiResponse?.data?.data)
            // settotalTransaction(ApiResponse?.data?.data[4][0]);
    ...
    const onFinish = async (values) => {
        try {
            ...
            let ApiResponse = await Post_Cooperative({ ...values, terminal_puv_type_id: selected_puv_type_id })
            Setisfetching(true)
    ...
    const handleDelete = async (record) => {
        try {
            ...
            let ApiResponse = await Delete_Cooperative(record?.id)
    ...
    const onFinishEdit = async (values) => {
        try {
            let ApiResponse = await Put_Cooperative({
                ...values, id: CurrentRow?.id,terminal_puv_type_id: selected_puv_type_id
            })
    ...
    const columns = [
        ...
        {
            title: 'Puv Type  Name',
            dataIndex: 'terminal_puv_type_name',
            key: 'terminal_puv_type_name',
            render: (dom, entity) => {
                return dom
        ...
        <Layout>
...

            {
                ...
                !isfetching ?
                    <>
                        ...
                        <div>
                            <h1 className="text-center text-purple-600">Cooperatives</h1>
            ...
            >
                ...
                >
                    ...
                    >
                        <Select showSearch size='large' name="terminal_puv_type_id" onChange={(e) => {
                            set_selected_puv_type_id(puv_type_data?.filter((item) => {
                                if (e == item?.id) {
                                    return item?.id
                                }
                            })?.[0]?.id)
