...
import { Delete_Puv, Get_Puv, Post_Puv, Put_Puv } from "../../api/puv";
...
export default function Home() {
    ...
    const [selected_coop_id, set_selected_coop_id] = useState(null);
    const [coop_data, set_coop_data] = useState([]);
    ...
    const Handle_Get_Cooperative= async () => {
        Setisfetching(true)
        ...
        try {
            ...
            set_coop_data(ApiResponse?.data?.data)
            // settotalTransaction(ApiResponse?.data?.data[4][0]);
    ...
    const onFinish = async (values) => {
        try {
            ...
            let ApiResponse = await Post_Puv({ ...values, cooperative_id: selected_coop_id })
            Setisfetching(true)
    ...
    const handleDelete = async (record) => {
        try {
            ...
            let ApiResponse = await Delete_Puv(record?.id)
    ...
    const onFinishEdit = async (values) => {
        try {
            let ApiResponse = await Put_Puv({
                ...values, id: CurrentRow?.id,terminal_puv_type_id: selected_coop_id
            })
        ...
        <Layout>
...

            {
                ...
                !isfetching ?
                    <>
                        ...
                        <div>
                            <h1 className="text-center text-purple-600">PUV</h1>
            ...
            >
                ...
                >
                    ...
                    >
                        <Select showSearch size='large' name="cooperative_id" onChange={(e) => {
                            set_selected_coop_id(coop_data?.filter((item) => {
                                if (e == item?.id) {
                                    return item?.id
                                }
                            })?.[0]?.id)
