...
export default function Home() {
    ...
    const [area_id, set_area_id] = useState(null);
    ...
    const handleGetData = async () => {
        ...
        try {
            ...
            let ApiResponse = await GetSection()
    ...
    const onFinish = async (values) => {
        try {
            ...
            let ApiResponse = await postSection({ ...values, area_id: area_id })
            Setisfetching(true)
    ...
    const handleDelete = async (record) => {
        try {
            ...
            let ApiResponse = await deleteSection(record?.id)
    ...
    const onFinishEdit = async (values) => {
        try {
            let ApiResponse = await putSection({
                ...values, area_id: area_id, id: CurrentRow?.id
            })
        ...
        <Layout>
...

            {
                ...
                !isfetching ?
                    <>
                        ...
                        <div>
                            <h1 className="text-center text-purple-600">Sections</h1>
            ...
            >
                ...
                >
                    ...
                    >
                        <Select showSearch size='large' name="area_id" onChange={(e) => {
                            set_area_id(AreaData?.filter((item) => {
                                if (e == item?.name) {
                                    return item?.name
                                }
                            })?.[0]?.id)
                    ...
                    <Form.Item
                        label="Rental Fee/month"
                        name="rent_per_month"

                        rules={[
                            {
                                required: true,
                                message: 'Please input your rent_per_month name!',
                            },
                        ]}
                    >
